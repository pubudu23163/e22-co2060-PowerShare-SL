...
router.post('/', auth, async (req, res) => {
  try {
    ...
    // If auto-confirmed → update host earnings immediately
    if (!isManual && charger.ownerId) {
      const hostEarning = (req.body.totalPrice || 0) * 0.90;
      await User.findByIdAndUpdate(charger.ownerId, {
    ...
    if (isManual) {
      if (charger.ownerId) {
        await createNotification({
          ...
          message: `${req.user.name} wants to book "${charger.name}" — ${req.body.date} at ${req.body.time}`,
          type: 'booking',
...
router.patch('/:id/accept', auth, async (req, res) => {
  try {
    ...
    if (String(booking.hostId) !== String(req.user.userId))
    ...
    const hostEarning = (booking.totalPrice || 0) * 0.90;
...
router.patch('/:id/cancel', auth, async (req, res) => {
  try {
    ...
    // Reverse earnings if was confirmed
